angular.module('naukri.listing', [])
    .directive('ngRepeatDoneNotification', function() {
        ...
        return {
            ...
            template: '<ul>' +
                '<li ng-repeat="item in data|limitTo:tupleCount|filter:{$:filterName}" ng-repeat-done-notification={{item.id}} ng-click="checkItem(this.item,$event)" ng-class="{notSelectable:item.notSelectable}">' +
                '<input ng-if="multiSelect == \'true\' && !item.notSelectable" type="checkbox" ng-checked=isChecked(item)>{{item.name}}' +
                '<listing filter-name="filterName" tuple-count="10" multi-select="true" parent="item" ng-if="item.list" selected-id ="selectedId" listing-callback="callbackRef(item)" callback-ref="callbackRef" data="item.list" selected-items="selected_items"><div><span>{{$parent.item.name}}</span></div></listing>' +
                '</li>' +
                '</ul>',
            compile: function(tElement, tAttr, trans) {
                ...
                return function(scope, iElement, iAttr, controllers) {
                    ...
                    scope.firstReapet = 0;
                    scope.selObj = {};
                    scope.idHash = [], scope.selectedItems = {};
                    ...
                    function checkParent(sc) {
                        ...
                        if (flag) {
                            if (sc.selectedId.indexOf(sc.parent.id) < 0) sc.selectedId.push(sc.parent.id);
                        } else {
